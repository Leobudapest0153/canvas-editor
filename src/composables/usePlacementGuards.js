...
export function usePlacementGuards({ store, alturaBodega, CM_TO_PX }) {
  ...
  const installDragBounds = (node, elementId) => {
    ...
    const lastAbs = { ...node.getAbsolutePosition() }
    const bound = (posLocal) => {
      ...
      let x = cArea.x
      let y = cArea.y
      const hint = { x: x - lastAbs.x, y: y - lastAbs.y }
      const res = resolveZStackClamp({ ...base, x, y }, neighbors, areaBounds, 0.001, hint)
      if (res.collided && res.corrected) {
        x = res.corrected.x
        y = res.corrected.y
      } else {
        lastGoodPos.set(elementId, { x, y })
      }
      lastAbs.x = x
      lastAbs.y = y
