...
export function validateCoplanarSameTypeNoOverlap(
  ...
  void Z_EPS
  const candType = resolveTypeId(element, candidate)
  const { zBaseCm: candZ } = resolveVerticalProps(element, candidate)
  if (!candType || candZ == null) return { valid: true }
  for (const other of neighbors) {
    ...
    if (!otherType || otherType !== candType) continue
    const { zBaseCm: oz } = resolveVerticalProps(other, {})
    if (oz == null || Math.abs(oz - candZ) > Z_LAYER_EPS) continue
    if (
...
export function resolveCoplanarClamp(candidate, neighbors, Z_LAYER_EPS = 0.5) {
  const candType = resolveTypeId(candidate, {})
  const { zBaseCm: candZ } = resolveVerticalProps(candidate, {})
  if (!candType || candZ == null) return {}
  let best = null
  for (const other of neighbors) {
    if (other.id === candidate.id) continue
    if (other.plantaId && candidate.plantaId && other.plantaId !== candidate.plantaId) continue
  ...
  if (best) {
    return { corrected: { x: candidate.x + best.dx, y: candidate.y + best.dy } }
  }
  return {}
}
