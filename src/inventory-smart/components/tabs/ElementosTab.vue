...
const selectedCatalog = computed({
  get: () => catalogStore.selectedCatalog,
  set: (val) => catalogStore.setSelectedCatalog(val),
})
...
onMounted(() => {
  if (typeof window !== 'undefined') {
    const stored = window.localStorage.getItem('inventory.selectedCatalog')
    if (stored === 'plantillas' || stored === 'elementos') {
      catalogStore.setSelectedCatalog(stored)
    }
  }
})
...
watch(selectedCatalog, (val) => {
  if (typeof window !== 'undefined') {
    window.localStorage.setItem('inventory.selectedCatalog', val)
  }
})
</script>
