...
export function applyEdgeConstraint(candidate, el, areaBounds, opts = {}) {
  ...
  const isInfinite =
    !!(areaBounds && areaBounds.polygon && areaBounds.polygon._isInfinite === true) ||
    areaBounds?.mode === 'elastic'

  // Bypass de límites si la planta es infinita (consultar polígono activo si existe o modo elastic)
  if (isInfinite) {
