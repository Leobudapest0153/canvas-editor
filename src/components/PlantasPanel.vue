...
<template>
  <div class="bg-white border-b border-gray-200 p-4 shadow-sm" style="overflow: visible">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 items-center" style="overflow: visible">
      ...
      <div
        class="flex flex-row overflow-x-auto space-x-4 col-span-1 lg:col-span-8"
      ...
      >
        ...
        <div
          class="sticky right-0 z-10 pl-2 -mr-2 bg-gradient-to-l from-white via-white/70 to-transparent flex items-center pointer-events-none"
        >
          <button
      ...
      <div
        class="grid grid-cols-1 sm:grid-cols-2 gap-2 col-span-1 lg:col-span-4 lg:justify-self-end"
      >
        <button
          type="button"
  ...
  try {
    ...
    if (res.status === 'block') {
      window.__toasts?.show?.(
        'No es posible reducir: elementos no caben con las nuevas dimensiones',
        { type: 'error' },
      )
      return
    }
    ...
    if (mostrarModalEditar.value) {
      ...
      // Candidatos: solo elementos de nivel raíz (sin padre) que van en suelo
      const candidates = canvasStore.elementos.filter(
        (e) =>
          ...
          !(e.decorativo && /\bsuelo\b/i.test(e.nombre || '')),
      )
      ...
      if (anyOut) {
        ...
        if (!placements) {
          ...
          window.__toasts?.show?.(
            'No fue posible reacomodar elementos; se revierte la redimensión',
            { type: 'error' },
          )
